...
{
    int32_t            nSegments;         //!< Number of segments
    char**         szSegmentNames;    //!< Array of segment names
    int32_t*           iParents;          //!< Array of segment parents (defines the hierarchy)
...
typedef struct sBodyData
{
    ...
    int32_t        ZoomEncoderValue;     //!< Zoom value from the Camera Tracker Encoder
    int32_t        FocusEncoderValue;    //!< Focus value from the Camera Tracker Encoder
...
typedef struct sAnalogData
{
    ...
    int16_t*       AnalogSamples;    //!< The data: nChannels * nSamples of these
...
{
    int32_t        bRecording;   //!< 0=Not Recording, anything else=Recording
    int32_t        iFirstFrame;  //!< The frame number of the first data frame to be recorded from Cortex Live Mode
    int32_t        iLastFrame;   //!< The frame number of the last data frame to be recorded from Cortex Live Mode
...
{
    int32_t        iFrame;                  //!< Cortex's frame number
...
*/
int Cortex_Initialize(const char* szCortexNicCardAddress);  // Hostname or IP Address or NULL (auto find)
...
int Cortex_Request(const char* szCommand, void** ppResponse, int *pnBytes, double timeout);  // Friendly extendable command function.
...
sBodyDefs*     Cortex_GetBodyDefs(double timeout);      // The description of what we are tracking.

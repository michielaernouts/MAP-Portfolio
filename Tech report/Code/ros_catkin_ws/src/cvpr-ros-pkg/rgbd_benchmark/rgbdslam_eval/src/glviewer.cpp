...
void GLViewer::pointCloud2GLStrip(pointcloud_type const * pc){
	...
	if (w*h == 0)
	{
		ROS_WARN("strip with empty cloud");
		return;
	}

	int step = ParameterServer::instance()->get<int>("rendering_step");

	for(unsigned int y = 0; y < h-step; y += step){ //go through every point and make two triangles
		for(unsigned int x = 0; x < w-step; x += step ){//for it and its neighbours right and/or down
			...
			if(!strip_on){ //Generate vertices for new triangle
				...
				if(hasValidXYZ(*ul)){ //ul, ur, ll all valid
					...
					// ROS_INFO("dist: %f, depth: %f, di/de: %f t: %f", squaredEuclideanDistance(*ul,*ll), depth,squaredEuclideanDistance(*ul,*ll)/depth, mesh_thresh);
...
void GLViewer::toggleTriangulation() {
	...
	if(polygon_mode == GL_FILL){ // Turn on Pointcloud mode
		ROS_INFO("Toggling Triangulation to GL_POINT");
		polygon_mode = GL_POINT;
	} else if(polygon_mode == GL_POINT){ // Turn on Wireframe mode
		polygon_mode = GL_LINE;
		ROS_INFO("Toggling Triangulation to GL_LINE");
	} else { // Turn on Surface mode
		polygon_mode = GL_FILL;
		ROS_INFO("Toggling Triangulation to GL_FILL");
	}

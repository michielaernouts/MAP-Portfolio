    										...
    										batch_processing_runs_(false)
{
	...
	optimizer_file.open("optimizer_timing.txt");
	assert(optimizer_file.is_open());
...
std::vector<int> GraphManager::getPotentialEdgeTargets(const Node* new_node, int max_targets){
	const int last_targets = 10; //always compare to the last n, spread evenly for the rest
...
AddNodeReturnType GraphManager::addNode(Node* new_node) {
	std::clock_t starttime=std::clock();
	...
	if (graph_.size()==0){
		...
		//pcl::fromROSMsg(*cloud_ptr,pc_local);

		//cout << "pc.sze, w, h " << pc_local.points.size() << " " << pc_local.width << " "<< pc_local.height << " " << endl;

		//pointcloud_type const * the_pc(&pc_local);
		// Q_EMIT setPointCloud(the_pc, latest_transform_);
		//cout << "pointcloud set" << endl;
	...
	ROS_INFO_STREAM_COND_NAMED(( (std::clock()-starttime) / (double)CLOCKS_PER_SEC) > ParameterServer::instance()->get<double>("min_time_reported"), "timings", "addNode runtime: "<< ( std::clock() - starttime ) / (double)CLOCKS_PER_SEC  <<"sec");
...
#define QT_NO_CONCURRENT
	...
	for(int i = 0; i <  results.size(); i++){
		...
		if(mr.edge.id1 >= 0){
			if (addEdgeToHogman(mr.edge, isBigTrafo(mr.edge.mean))) { //TODO: result isBigTrafo is not considered
				...
				added_edges.push_back(mr.edge.id1);
				// ROS_INFO("Added Edge between %i and %i. Inliers: %i",mr.edge.id1,mr.edge.id2,(int) mr.inlier_matches.size());
				//last_matching_node_ = mr.edge.id1;
				//last_inlier_matches_ = mr.inlier_matches;
				//last_matches_ = mr.all_matches;
	...
	ROS_INFO("Added Connections to %d nodes", (int) added_edges.size());
	...
	if (optimizer_->edges().size() > num_edges_before) { //Success
		...
		// test: downsample cloud to decrease memory footprint
		/*pcl::VoxelGrid<point_type> downsampler;
        downsampler.setInputCloud(the_pc);
        downsampler.setLeafSize(0.01, 0.01, 0.01);
        downsampler.filter(cloud);
        ROS_INFO("PointCloud after filtering: %d data points.", the_pc->width * the_pc->height);
		 */
		// end of test
...
void GraphManager::optimizeGraph(){
	...
	// write optimizer size and running time to file
	optimizer_file << optimizer_->vertices().size() << " " << optimizer_->edges().size() << " " << ((endtime-starttime) / (double)CLOCKS_PER_SEC) << endl;
	...
	ROS_INFO_STREAM_COND_NAMED(( (endtime-starttime) / (double)CLOCKS_PER_SEC) > ParameterServer::instance()->get<double>("min_time_reported"), "timings", "Optimizer Runtime: "<< ( endtime - starttime ) / (double)CLOCKS_PER_SEC  <<"sec");
}

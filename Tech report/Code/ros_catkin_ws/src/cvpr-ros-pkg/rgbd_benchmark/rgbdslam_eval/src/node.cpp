...
matcher_(matcher)
{
  ...
  pointcloud_type pc_local;

  // use local cloud to only generate the feature points (no visualization, but mem efficient)
  //pc_col = new pointcloud_type();
  // pcl::fromROSMsg(*point_cloud,pc_col);
  pcl::fromROSMsg(*point_cloud,pc_local);
  ...
  projectTo3DSiftGPU(feature_locations_2d_, feature_locations_3d_, pc_local, descriptors, feature_descriptors_); //takes less than 0.01 sec

  if (descriptors != NULL) delete descriptors;

#else
  projectTo3D(feature_locations_2d_, feature_locations_3d_, pc_local); //takes less than 0.01 sec
#endif
...
/*
  ...
  sor.filter (filtered);
  pcl::fromROSMsg(filtered,pc_col);
*/
...
//  ROS_INFO("Use extractor");
 ...
 // cout << "2d pos " << feature_locations_2d_.size() << endl;
 // cout << "3d pos " << feature_locations_3d_.size() << endl;
 // cout << "desciptors: " << feature_descriptors_.rows << " x " << feature_descriptors_.cols << endl;
...
void Node::computeInliersAndError(const std::vector<cv::DMatch>& matches,
	...
	//dists.resize(matches.size());
	//errors.resize(matches.size());
	...
	for (unsigned int j = 0; j < matches.size(); j++){ //compute new error and inliers
		...
		vec = (transformation * origins[this_id]) - earlier[earlier_id];

		error = vec(0)*vec(0)+vec(1)*vec(1)+vec(2)*vec(2);
		...
		error = sqrt(error);
		// dists[pos] = (pair<float,int>(error,j));

		inliers[inlier_cnt] = matches[j]; //include inlier
		inlier_cnt++;
	...
	if (inlier_cnt<3){ //at least the samples should be inliers
		ROS_WARN_COND(inlier_cnt > 3, "No inliers at all in %d matches!", (int)matches.size()); // only warn if this checks for all initial matches
		mean_error = 1e9;
	}

	inliers.resize(inlier_cnt);
	mean_error /= inlier_cnt;
	...
	*/
	...
	// if(!(mean_error>0)) ROS_ERROR_STREAM(mean_error << " " << inliers.size());
	// ROS_INFO_STREAM_COND_NAMED(( (std::clock()-starttime) / (double)CLOCKS_PER_SEC) > ParameterServer::instance()->get<double>("min_time_reported"), "timings", "computeInliersAndError runtime: "<< ( std::clock() - starttime ) / (double)CLOCKS_PER_SEC  <<"sec");
	// ROS_INFO("computeInliers: %.3f s", (std::clock()-starttime) / (double)CLOCKS_PER_SEC);
}
                                       ...
                                       std::vector<cv::DMatch>& matches) const
{
  ...
  if(initial_matches->size() <= (unsigned int) ParameterServer::instance()->get<int>("min_matches")){
	...
	const double max_dist_m = ParameterServer::instance()->get<double>("max_dist_for_inliers");


	// true if ransac should maximize number of inliers, false if it should minimize the errror of all inliers
	const bool optimize_inlier = ParameterServer::instance()->get<bool>("RANSAC_optimize_inliers");
	bool optimize_error = !optimize_inlier;

	/* if (optimize_inlier)
    ROS_INFO("Optimizing Inlier cnt");
  else
    ROS_INFO("Optimizing Inlier error"); */

	vector<double> dummy;

	// best values of all iterations (including invalids)
	double best_error = 1e6, best_error_invalid = 1e6;
	unsigned int best_inlier_invalid = 0, best_inlier_cnt = 0;
	...
	// float pct = 0.3;
	// uint min_match_cnt = 20;
	double pct = ParameterServer::instance()->get<double>("min_inlier_percentage");
	int min_match_cnt = ParameterServer::instance()->get<int>("min_match_cnt");
	uint max_ransac_iterations = ParameterServer::instance()->get<int>("max_ransac_iterations");

	uint min_inlier_threshold = int(initial_matches->size()*pct);
	// min_inlier_threshold = min(min_inlier_threshold,min_fix);


	if(initial_matches->size()*pct <= min_match_cnt){
		// ROS_INFO("Only %d potential matches between %d and %d (minimal: %i)",(int)initial_matches->size() , this->id_, earlier_node->id_, int(min_match_cnt*1.0/pct));
		return false;
	}


	uint valid_iteration_cnt = 0;

	// break after so many valid iterations
	uint max_valid_iterations = ParameterServer::instance()->get<int>("max_valid_iterations");

	// ROS_INFO("start Ransac with %i potential matches and threshold at %i", (int)initial_matches->size(), min_inlier_threshold);

	for (uint n_iter = 0; n_iter < max_ransac_iterations; n_iter++) {
		...
		/*computeInliersAndError(sample_matches_vector, transformation, this->feature_locations_3d_,
			  earlier_node->feature_locations_3d_, inlier, inlier_error,
		...
		if(inlier.size() < min_inlier_threshold ){best_inlier_invalid = inlier.size(); continue;}

		valid_iteration_cnt++;
		if (valid_iteration_cnt >= max_valid_iterations)
			break;
		...
		if ( ( optimize_inlier && (inlier.size() > best_inlier_cnt)) ||
				( optimize_error  && (inlier_error < best_error)) )
		{
	...
	//ROS_INFO("%i good iterations (from %i), inlier pct %i, inlier cnt: %i, error: %.2f cm",valid_iteration_cnt, (int) ransac_iterations, (int) (matches.size()*1.0/initial_matches->size()*100),(int) matches.size(),rmse*100);

	bool trafo_found = matches.size() >= min_inlier_threshold;

	if (!trafo_found)
		ROS_INFO("best overall: inlier: %i (min %i), error: %.2f",best_inlier_invalid, min_inlier_threshold, best_error_invalid*100);

	ROS_INFO_STREAM_COND_NAMED(( (std::clock()-starttime) / (double)CLOCKS_PER_SEC) > ParameterServer::instance()->get<double>("min_time_reported"), "timings", "RANSAC runtime: "<< ( std::clock() - starttime ) / (double)CLOCKS_PER_SEC  <<"sec");
	return trafo_found;
}

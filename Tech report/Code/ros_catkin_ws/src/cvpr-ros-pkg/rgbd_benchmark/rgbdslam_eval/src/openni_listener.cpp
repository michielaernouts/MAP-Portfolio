...
bool OpenNIListener::runBagFile(const char* bagfile_filename, const char* logfile_filename)
{

 char buffer[500];

 ROS_INFO("opening bagfile at %s", bagfile_filename);

 rosbag::Bag bag;
 bag.open(bagfile_filename, rosbag::bagmode::Read);

 ROS_INFO("writing log to %s", logfile_filename);
 path_file.open(logfile_filename);
 assert(path_file.is_open());

 std::vector<std::string> topics;
 topics.push_back(std::string("/camera/rgb/image_mono"));
 ...
 BagSubscriber<sensor_msgs::Image> mono_sub, depth_sub;
 BagSubscriber<PointCloud2> pc_sub;
 message_filters::Synchronizer<MySyncPolicy> sync(25,mono_sub, depth_sub,pc_sub);
 // message_filters::TimeSynchronizer<sensor_msgs::Image, sensor_msgs::Image, sensor_msgs::PointCloud2> sync(mono_sub, depth_sub,pc_sub, 25);
 // sync.registerCallback(boost::bind(&callback, _1, _2, _3));
 sync.registerCallback(boost::bind(&OpenNIListener::cameraCallback, this, _1, _2, _3));
 ...
 BOOST_FOREACH(rosbag::MessageInstance const m, view)
 {
   ...
   {
    //ROS_INFO("t0");
    mono_sub.newMessage(m.instantiate<sensor_msgs::Image>());
   }

   if (m.getTopic() == topics[1])
   {
    //ROS_INFO("t1");
    depth_sub.newMessage(m.instantiate<sensor_msgs::Image>());
   }

   if (m.getTopic() == topics[2])
   {
    // ROS_INFO("t2");
    pc_sub.newMessage(m.instantiate<sensor_msgs::PointCloud2>());
   }
 ...
 sensor_msgs::Image::ConstPtr* visual_img = new sensor_msgs::Image::ConstPtr();
 sensor_msgs::Image::ConstPtr* depth_img = new sensor_msgs::Image::ConstPtr();
 sensor_msgs::PointCloud2::ConstPtr* cloud = new sensor_msgs::PointCloud2::ConstPtr();
 ...
 BOOST_FOREACH(rosbag::MessageInstance const m, view)
 {
  if (m.getTopic() == topics[0])
  {
   *visual_img = m.instantiate<sensor_msgs::Image>();
   ROS_INFO("got msg with type %s \t at %li (img: %li)", m.getTopic().c_str(),
            static_cast<long>(m.getTime().toNSec()),
            static_cast<long>((*visual_img)->header.stamp.toNSec()));
  ...
  if (m.getTopic() == topics[1])
  {
   *depth_img = m.instantiate<sensor_msgs::Image>();
   ROS_INFO("got msg with type %s \t at %li (img: %li)", m.getTopic().c_str(),
            static_cast<long>(m.getTime().toNSec()),
            static_cast<long>((*depth_img)->header.stamp.toNSec()));
  ...
  if (m.getTopic() == topics[2])
  {
   *cloud = m.instantiate<sensor_msgs::PointCloud2>();
   ROS_INFO("got msg with type %s \t at %li (img: %li)", m.getTopic().c_str(),
            static_cast<long>(m.getTime().toNSec()),
            static_cast<long>((*cloud)->header.stamp.toNSec()));
  ...
  if (msgs_arrived == 3)
  {
   ROS_INFO("got all msgs, callback");
   cameraCallback(*visual_img, *depth_img,*cloud);
   msgs_arrived = 0;
  }
*/
...
//pc_pub(nh.advertise<sensor_msgs::PointCloud2>("transformed_cloud", 2))
{
 ...
 // open file for writing the path
// strftime (buffer,80,"path/path_%Y_%m_%d__%H_%M_%S.txt",timeinfo);
// path_file.open(buffer);
// assert(path_file.is_open());
...
void OpenNIListener::cameraCallback (const sensor_msgs::ImageConstPtr& visual_img_msg, 
 ...
 ROS_INFO("Received data from kinect: %i ", callback_counter++);
/*
 ros::Time depth_time = depth_img_msg->header.stamp;
 ros::Time color_time = visual_img_msg->header.stamp;
 ros::Time cloud_time = point_cloud->header.stamp;
 ROS_INFO("depth: %li", static_cast<long>(depth_time.toNSec()));
 ROS_INFO("color: %li", static_cast<long>(color_time.toNSec()));
 //ROS_INFO("cloud: %li", static_cast<long>(cloud_time.toNSec()));
 ROS_INFO("abs(d-c):    %li", abs(static_cast<long>(color_time.toNSec()-static_cast<long>(depth_time.toNSec()))));


 ROS_INFO("c_t-c_{t-1}:  %li", abs(static_cast<long>(color_time.toNSec()-static_cast<long>(last_rgb_time.toNSec()))));
 ROS_INFO("d_t-d_{t-1}:  %li", abs(static_cast<long>(depth_time.toNSec()-static_cast<long>(last_depth_time.toNSec()))));

 last_rgb_time = color_time;
...
*/
 ...
 if(d_time.nsec != pc_time.nsec || rgb_timediff > 33333333){
  ...
  //ROS_INFO("async: Depth image time: %d - %d", d_time.sec,   d_time.nsec);
  //ROS_INFO("async: RGB   image time: %d - %d", rgb_time.sec, rgb_time.nsec);
  //ROS_INFO("async: Point cloud time: %d - %d", pc_time.sec,  pc_time.nsec);
...
void OpenNIListener::processNode(cv::Mat& visual_img,   // will be drawn to
 ...
 AddNodeReturnType action = graph_mgr_->addNode(new_node);


 // pointcloud_type const * the_pc(&(new_node->pc_col));
 // Q_EMIT graph_mgr_->setPointCloud(the_pc, graph_mgr_->latest_transform_);

 // new_node->pc_col.points.resize(0);

 //	bool save_cloud = ParameterServer::instance()->get<bool>("store_point_cloud");
 //	if (!save_cloud)
 {
  //	new_node->pc_col.width = new_node->pc_col.width = 0;
  //	new_node->pc_col.points.clear();
 }
 ...
 {
  cnt_added++;
  AISNavigation::PoseGraph3D::Vertex* v = graph_mgr_->optimizer_->vertex(graph_mgr_->optimizer_->vertices().size()-1);

  //pcl_ros::transformAsMatrix(kinect_transform_, latest_transform_);
  //latest_transform_ = hogman2QMatrix(v->transformation);

  path_file << new_node->time_of_pointcloud_msg << ", ";
  path_file << v->transformation.translation().x() << ", ";
  path_file << v->transformation.translation().y() << ", ";
  path_file << v->transformation.translation().z() << ",    ";

  path_file << v->transformation.rotation().x() << ", ";
  path_file << v->transformation.rotation().y() << ", ";
  path_file << v->transformation.rotation().z() << ", ";
  path_file << v->transformation.rotation().w() << endl;
 ...
 {
  cnt_irrelevant++;
  path_file << new_node->time_of_pointcloud_msg << ", 42, 0, 0,    0, 0, 0, 0" << endl;
 }

 if (action == FAILED)
 {
  cnt_failed++;
  path_file << new_node->time_of_pointcloud_msg << ", 23, 0, 0,    0, 0, 0, 0" << endl;
 }


 ROS_INFO("processNode with callback nr %d, added: %d, irrelevant: %d, failed: %d, sum: %d ", callback_counter, cnt_added, cnt_irrelevant, cnt_failed,cnt_added+cnt_irrelevant+cnt_failed);



 bool has_been_added = (action == ADDED);

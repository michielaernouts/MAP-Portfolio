...
void ParameterServer::parseConfig() {
  ...
  config["start_paused"]                 =  static_cast<bool>  (0);
  config["subscriber_queue_size"]        =  static_cast<int>   (10);
  ...
  // distance threshold between consecutive poses
  config["min_translation_meter"]        =  static_cast<double>(0);
  config["min_rotation_degree"]          =  static_cast<int>   (3);
  ...
  config["drop_async_frames"]            =  static_cast<bool>(0);

  // parameters for node.cpp getRelativeTransformationTo
  config["RANSAC_optimize_inliers"]	 =  static_cast<bool>(1);
  config["min_inlier_percentage"] 	 =  static_cast<double>(0.25);
  config["min_match_cnt"] 		     =  static_cast<int>(20);
  config["max_valid_iterations"] 	 =  static_cast<int>(500);
  config["max_ransac_iterations"] 	 =  static_cast<int>(20000);

  // parameters for plotting:
  // only use every n.th horizontal and vertical point
  // speedup for drawing (if not needed, set to 1)
  config["rendering_step"] 		     =  static_cast<int>(1);

  // if false, the node doesn't store the pc (frees memory)
  config["store_point_cloud"]        =  static_cast<bool>(1);
